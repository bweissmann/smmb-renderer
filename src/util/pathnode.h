...
struct PathNode {
    ...
    PathNode (const Ray &ray, const Eigen::Vector3f &contrib, const Eigen::Vector3f &hit, const Eigen::Vector3f &left_from,
        ...
        : outgoing_ray(ray), contrib(contrib), hit_position(hit), left_from(left_from), surface_normal(normal), type(type), mat(mat), directional_prob(directional_prob), point_prob(point_prob){}
